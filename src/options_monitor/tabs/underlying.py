...
from options_monitor.charts.es_chart import build_es_candlestick_chart
from options_monitor.charts.spx_chart import build_spx_candlestick_chart
from options_monitor.config import OPTIONS_DIR, SCHWAB_CANDLE_DIR
...
def _load_gex_levels(
    ...
    zgl = find_zero_gamma_level(price_gex["price"].to_numpy(), price_gex["net_gex"].to_numpy())
...
def render_underlying_tab(candle_dir: Path) -> None:
    ...
    with col_ctrl:
        ticker_label = st.selectbox("Ticker", _TICKER_OPTIONS, index=1, key="reg_ticker") or "SPX"
        freq = (
